        ...
        <div class="InfoWrap">
          <div class="goodsDetail">
            ...
            <div class="support">
              ...
              <div class="supportArea">
                ...
                <div class="fixWidth">广东省 深圳市 福田区</div>
              </div>
            </div>
          </div>
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前的咱们的路由跳转 是从a路由跳转到b路由 这里的购物车跳转之前需要发请求 
                  把你购买的产品通过请求的形式通知服务器,服务器进行相应的存储
                -->
...
export default {
  ...
  // 当前组件挂载完毕时获取产品详情信息
  mounted(){
    //派发action请求 并接收search组件传递过来的商品id 在挂载前接收
    // 这里注意接收的params参数必须跟定义路由规则里的占位符的名字必须是一样的
    this.$store.dispatch("getGoodInfo", this.$route.params.skuId);
  },
  computed: {
    ...mapGetters(["categoryView", "skuInfo", "spuSaleAttrList"]),
    // 给子组件的数据
    // 如果服务器的数据没有回来skuinfo这个对象时空对象
    skuImageList() {
  ...
  methods:{
   ...
   // 表单元素修改产品的个数
    ...
    //  如果用户输入进来的是非法的出现NAN或者小于1 
      if(isNaN(value)||value<1){
        this.skuNum=1
      }else{
        // 正常的就是大于一的但是不能是小数所以向上取整
   ...
   async addShopCar(){
      ...
      //  3:失败 给用户进行提示
      // 当前这里是派发了一个action也向服务器发送请求了 判断加入购物车加入成功还是失败
      // 这行代码说白了就是调用仓库中的addOrUpdateShopCart函数
    try{
      await this.$store.dispatch('addOrUpdateShopCart',{
        skuId:this.$route.params.skuId,skuNum:this.skuNum
      });
      // 如果成功路由跳转
      // 在路由进行跳转的时候还需要把产品的信息带给下一级的路由组件
      // 一些简单的数据通过query形式给路由组件传递过去 复杂的数据通过浏览器会话存储 好处是用完就消失数据
      // 不管是本地存储还是会话存储一般存的是字段窜  不能穿镀对象
