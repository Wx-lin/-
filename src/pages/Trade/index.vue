  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div
        ...
        :key="address.id"
      >
        <!-- 使用动态类名来展示isDefault是否为1来判断展示或者不展示 -->
        <span class="username" :class="{selected:address.isDefault == 1 }">{{
          address.consignee
        }}</span>
        <!-- 这块的回调函数也不可以不传参数因为它的组件身上本来就有 -->
...
// import { render } from 'nprogress';
    ...
    computed:{
       ...
       ...mapState({
        ...
        //  在组件中拿到orderInfo里的数据并进行动态展示
        orderInfo:state=>state.trade.orderInfo,
         }),
         //将来提交订单最终选中的地址
         userDefaultAddress(){
          //  find是查找数组当中符合条件的元素返回作为最终的结果 
          // 第二种情况就是有可能在判断条件的时候数据还没有拿到就会报错所以加个或者是一个数组,最起码空数组不会报错
    ...
    methods:{
      ...
      changeDefault(address,addressInfo){
            // 把数组里的元素遍历一遍让全部的isDefault为0让当前点击的为1
     ...
     async submitOrder(){
            ...
            let data={
              ...
              let result=await  this.$API.reqSunmitOrder(tradeNo,data);
                // 提交订单成功
              if(result.code==200){
